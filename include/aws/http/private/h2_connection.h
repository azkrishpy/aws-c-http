...
struct aws_h2_connection {
    ...
    struct {
        ...
        /* Connection-level statistics. Note there is deliberately no per-stream monitor: health is judged for the
         * connection as a whole, since that is the unit the channel can shut down. */
