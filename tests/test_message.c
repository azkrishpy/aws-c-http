...
TEST_CASE(message_h2_from_http1_origin_form_target) {
    ...
    ASSERT_SUCCESS(
        s_h2_pseudo_header_from_http1(allocator, "GET", "/index.html", host_header, 1, ":scheme", &value, &found));
    ASSERT_TRUE(found);
    ASSERT_TRUE(aws_byte_buf_eq_c_str(&value, "https"));
    aws_byte_buf_clean_up(&value);

    ASSERT_SUCCESS(
        s_h2_pseudo_header_from_http1(allocator, "GET", "/index.html", host_header, 1, ":authority", &value, &found));
    ASSERT_TRUE(found);
...
TEST_CASE(message_h2_from_http1_absolute_form_target) {
    ...
    /* An explicit Host header still wins over the target's authority */
    struct aws_http_header host_header[] = {DEFINE_HEADER("Host", "host-header.example")};
    ASSERT_SUCCESS(s_h2_pseudo_header_from_http1(
        allocator, "GET", "http://target.example/index.html", host_header, 1, ":authority", &value, &found));
    ASSERT_TRUE(found);
    ASSERT_TRUE(aws_byte_buf_eq_c_str(&value, "host-header.example"));
    aws_byte_buf_clean_up(&value);
...
TEST_CASE(message_h2_from_http1_asterisk_form_target) {
    ...
    ASSERT_SUCCESS(s_h2_pseudo_header_from_http1(allocator, "OPTIONS", "*", host_header, 1, ":scheme", &value, &found));
    ASSERT_TRUE(found);
    ASSERT_TRUE(aws_byte_buf_eq_c_str(&value, "https"));
    aws_byte_buf_clean_up(&value);
